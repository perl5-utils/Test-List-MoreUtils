...
@in = @even;
@expected = mesh @odd, @even;
foreach my $v (reverse @odd) { binsert { $_ <=> $v } $v, @in }
...
@in = @odd;
foreach my $v (reverse @even) { binsert { $_ <=> $v } $v, @in }
...
leak_free_ok(
    ...
    'binsert odd into even' => sub {
        my @list = @even;
        foreach my $elem (@odd) { binsert { $_ <=> $elem } $elem, @list; }
    },
    'binsert even into odd' => sub {
        my @list = @odd;
        foreach my $elem (@even) { binsert { $_ <=> $elem } $elem, @list; }
    },
...
leak_free_ok(
    ...
    'binsert odd with stack-growing' => sub {
        my @list = @even;
        foreach my $elem (@odd) { binsert { grow_stack(); $_ <=> $elem } $elem, @list; }
    },
    'binsert even with stack-growing' => sub {
        my @list = @odd;
        foreach my $elem (@even) { binsert { grow_stack(); $_ <=> $elem } $elem, @list; }
    },
);
