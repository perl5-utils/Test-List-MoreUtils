...
SCOPE:
{
    ...
    my @out = arrayify @in;
    is_deeply( \@out, [qw(av_make av_undef av_clear av_push av_pop av_fetch av_store av_shift av_unshift)], "linear flattened string mix i" );
}
...
SKIP: {
    leak_free_ok(
        'arrayify with exception in overloading stringify at begin' => sub {
            my @in = (DieOnStringify->new, qw(av_make av_undef av_clear), [qw(av_push av_pop)], qw(av_fetch av_store), [['av_shift'], ['av_unshift']]);
            eval { my @out = arrayify @in; }; diag($@) if($@);
        },
    );
    skip "Broken in perl-5.26, tracked down to commit 8b0c3377", 1 if($^V ge v5.26.0);
    leak_free_ok(
